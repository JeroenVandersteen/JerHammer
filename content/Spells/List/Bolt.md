---
title: "Bolt"
titleIcon: "fa-solid fa-wand-sparkles"
draft: false
---

### Bolt

_The caster conjures a bolt of fire and hurls it at their opponent. For flavor, this may be changed to a bolt of lightning, ice, stone, dead birds, or whatever is stuiable for the character concept._

| Bolt |
| :--- |
| **Level:** 2 |
| **Lore:** Fire Element |
| **Casting TN:** 17 |
| **Type:** Combat |
| **Duration:** instant |
| **Range:** Short |
| **Target:** 1 |
| **Damage:** 6d6 L/i |
| **Armor:** AP/IR counts |
| **Resist:** Spell TN |
| **Effect:** Combat spell. Caster chooses lethal or impact |

This spell deals 6d6 L/i (AP/IR counts) to 1 target at short range instantly. The spellcast check has to also be equal to or higher than the target's Spell TN. Similar to Blast, but the damage is focused into a bolt of energy that strikes a single target.