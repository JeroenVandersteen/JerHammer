---
title: "Watchman"
titleIcon: "fa-solid fa-compass-drafting"
draft: false
---


## Watchman

**Profession attributes**: WS, BS, P

**Profession skills**: Unarmed Combat, Weapon Offense, Weapon Defense, Bows, Crossbows, Athletics, Endurance, Sleight of Hand, Spot, Leadership

**Trappings**: Spear (1H), Torch, Torso leather, Head plate, Poor clothes, Cloak, Lantern, Lamp oil

### Level 1 (income: Poor)

| Trainings | Short Description |
| --- | --- |
| [Alertness](<Trainings/List/Alertness>) | Cannot be surprised |
| [Bar brawler](<Trainings/List/Bar brawler>) | +1d6 dam on unarmed/knuckles |
| [Drunken fighter](<Trainings/List/Drunken fighter>) | Opponents gain disadvantage |
| [Entangling strike](<Trainings/List/Entangling strike>) | Entangle when hitting |
| [Grappler](<Trainings/List/Grappler>) | Advantage when grappling |
| [Knockout strike](<Trainings/List/Knockout strike>) | Advantage to dam with impact weapons |
| [Consume alcohol](<Trainings/List/Consume alcohol>) | Advantage when consuming alcohol |


### Level 2 (income: Normal)

| Trainings | Short Description |
| --- | --- |
| [Armbreaker](<Trainings/List/Armbreaker>) | Opposed S test to break arm |
| [Drilled](<Trainings/List/Drilled>) | Dam +1 when next to ally |
| [Push away](<Trainings/List/Push away>) | Push when hitting with impact weapon |
| [Reliable hit](<Trainings/List/Reliable hit>) | Advantage on dam with that weapon |
| [Shield push](<Trainings/List/Shield push>) | Push with a shield |
| [Smash hard](<Trainings/List/Smash hard>) | 1d6 impact dam with lethal dam |
| [Lore](<Trainings/List/Lore>) (Local) | Deep knowledge on subject |


### Level 3 (income: Normal)

| Trainings | Short Description |
| --- | --- |
| [Bleeding strike](<Trainings/List/Bleeding strike>) | 1 fortune to add bleeding to damage |
| [Blinding strike](<Trainings/List/Blinding strike>) | 1 fortune to add blinded to damage |
| [Exploit advantage](<Trainings/List/Exploit advantage>) | Extra advantage on advantage |
| [Pull closer](<Trainings/List/Pull closer>) | Pull when hitting with entangle |
| [Stunning strike](<Trainings/List/Stunning strike>) | 1 fortune to add stun to damage |
| [Animal affinity](<Trainings/List/Animal affinity>) | Animals do simple things |
| [Investigative](<Trainings/List/Investigative>) | Gain clues |
| [Lore](<Trainings/List/Lore>) (Law) | Deep knowledge on subject |


### Level 4 (income: Normal)

| Trainings | Short Description |
| --- | --- |
| [Armorbreaker](<Trainings/List/Armorbreaker>) | Destroy armor instead of crit damage |
| [Brutal strike](<Trainings/List/Brutal strike>) | Add S to dam, opponent gains advantage to hit |
| [Bypass armor](<Trainings/List/Bypass armor>) | 1 fortune to ignore impact reduction |
| [Powerful strike](<Trainings/List/Powerful strike>) | -1 to hit, +1d6 damage |
| [Robust](<Trainings/List/Robust>) | +1 impact reduction |
| [Armor repair](<Trainings/List/Armor repair>) | Repair double armor points/day |
| [Lore](<Trainings/List/Lore>) (Politics) | Deep knowledge on subject |


